...
//         }
    ...
    void CXXMethodDeclToLua(CXXMethodDecl *set) {
        std::string mehodName = set->getQualifiedNameAsString();
        int paramNum = set->getNumParams();
        std::string newMethodDecl;
        newMethodDecl.append("function " + mehodName + "(");
        for(int i=0; i<paramNum; i++)
        {
            auto paramDecl = set->getParamDecl(i);
            std::string name = paramDecl->getNameAsString();
            newMethodDecl.append(name);
            if(i < (paramNum -1)) {
                newMethodDecl.append(", ");
            }
        }
        
        newMethodDecl.append(")");
        set->getBody();
        rewriter.ReplaceText(set->getBeginLoc(), 10, newMethodDecl);
    }
    
    bool VisitCXXMethodDecl(CXXMethodDecl *set) {
        //errs() << "VVVVVV!!!!: " << set->getQualifiedNameAsString() << "\n";
        std::string qualifiedName = set->getQualifiedNameAsString();
        ...
        if (qualifiedName == "Rectangle::set_values") {
            rewriter.ReplaceText(set->getQualifierLoc().getBeginLoc(), 9, "RectangleShape");
        }
//        if (qualifiedName == "Rectangle::fun1") {
//            Rewriter::RewriteOptions opt;
////            opt.IncludeInsertsAtBeginOfRange = false;
////            opt.IncludeInsertsAtEndOfRange = false;
//            opt.RemoveLineIfEmpty = true;
//            rewriter.RemoveText(set->getSourceRange(), opt);
//        }
        
        CXXMethodDeclToLua(set);
        return true;
    }
    bool VisitMemberExpr(MemberExpr *mem) {
//        errs() << "VVVVVV!!!!: " << mem->getMemberDecl()->getNameAsString() << "\n";
